...
cellValues = require("../../config/labyrinthConfig");
...
module.exports.getUserLabyrinth = (userId) => {
  ...
  module.exports.updateLabyrinthCell = async (id, xCord, yCord, type) => {
    ...
    if (doc && doc.matrix) {
      doc.matrix[xCord][yCord].value = Number.parseInt(cellValues[type]);
  ...
  module.exports.setLabyrinthStart = async (id, xCord, yCord) => {
    ...
    if (doc && doc.matrix) {
      doc.matrix[xCord][yCord].value = Number.parseInt(cellValues.start);
      // Save the updated document
      return doc.save();
    }
  };
  ...
  module.exports.setLabyrinthEnd = async (id, xCord, yCord) => {
    ...
    if (doc && doc.matrix) {
      doc.matrix[xCord][yCord].value = Number.parseInt(cellValues.end);
      // Save the updated document
      return doc.save();
    }
  };
